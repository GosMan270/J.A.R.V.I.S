...
DescriptionPrompt = {
    "standart": "Базовый режим. Модель просто отвечает на вопросы пользователя так, чтобы ответ был максимально естественным для последующего озвучивания голосом.",
    "database": "Режим обработки команд, связанных с личной базой данных — людьми, личными данными сайтов и личными медиа-сервисами. В этом режиме модель выбирает нужную таблицу, поле и действие, оформляя ответ в заданном формате с пояснением."
}
...
PromptSelectSystem = (
    ...
    f"{descriptions}\n"
    "Верни только ОДНО слово — название режима ('standart', 'database' и т.д., если не уверен — 'standart')."
)
